...
int run_vm(struct vm *vm, struct vcpu *vcpu, size_t sz)
{
	...
	uint32_t number_of_in = 0, number_of_out = 0;
	uint32_t GPA = 0;
	...
	for (;;) {
		...
		// counts total number of times the VM exited
		number_of_exits++;
		...
		case KVM_EXIT_IO:
			...
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT
				...
				// pointer to value from VM
				uint32_t* print_32bit_value = (uint32_t *)(p + vcpu->kvm_run->io.data_offset);
			...
			// for printing number of exits
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN 
				&& vcpu->kvm_run->io.port == 0xE5) {
				char *p = (char *)vcpu->kvm_run;
				uint32_t* offset_pointer = (uint32_t *)(p + vcpu->kvm_run->io.data_offset);
				
				// setting number of exits to the offset
				*offset_pointer = number_of_exits;
			...
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT 
				...
				// start address of VM
				char *starting_addr_of_vm = vm->mem;

				// address of string in guest
				uint32_t addr_of_str = *(uint32_t *)(p + vcpu->kvm_run->io.data_offset);
				
				// actual address in the hypervisor
				printf("%s", starting_addr_of_vm + addr_of_str);
			...
			// for HC_numExitsByType
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN 
				&& vcpu->kvm_run->io.port == 0xE3) {
				char *p = (char *)vcpu->kvm_run;

				number_of_in++;
				char num[15];
				bzero(num, 0);
				sprintf(num, "IO in: %d", number_of_in);

				// printing string on the guest address space
				int i=227 + x*1000;
				for(int j=0;j<(int)strlen(num);i++,j++)
				{
					*(vm->mem + i) = num[j];	
				}
				vm->mem[i] = '\n';
				...
				char num2[15];
				bzero(num2, 0);
				sprintf(num2, "IO out: %d", number_of_out);

				i++;

				// print remaining string
				for(int j=0;j<(int)strlen(num2);i++,j++)
				{
					*(vm->mem + i) = num2[j];	
				}
				vm->mem[i] = '\n';
				...
				// give the starting address back to VM
				uint32_t* offset_pointer = (uint32_t *)(p + vcpu->kvm_run->io.data_offset);
				*offset_pointer = 227 + x*1000;
			...
			// for HC_gvaToHva
			if ((vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT || vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN) 
				...
				if(vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT)
				{
					...
					// translate the GVA
					struct kvm_translation translate_GVA_TO_HVA;
					translate_GVA_TO_HVA.linear_address = *(uint32_t *)(p + vcpu->kvm_run->io.data_offset);
					if( ioctl(vcpu->vcpu_fd,KVM_TRANSLATE, &translate_GVA_TO_HVA) < 0)
					{
						perror("ioctl error");
						continue;
					}
					// set the GPA to below value
					if(translate_GVA_TO_HVA.valid)
					{	
						GPA = (uint32_t)(uintptr_t)vm->mem + translate_GVA_TO_HVA.physical_address;
					}
					else
					{
						GPA = 0;
				...
				{	
					// give back GPA to VM
					char *p = (char *)vcpu->kvm_run;
					*(uint32_t *)(p + vcpu->kvm_run->io.data_offset) = GPA;
				}	
