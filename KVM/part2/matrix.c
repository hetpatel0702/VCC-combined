...
void kvm_run_vm(struct vm *vm1, struct vm *vm2)
{
    ...
    timer_struct.it_value.tv_sec = QUANTUM;
    timer_struct.it_value.tv_nsec = 0;
    timer_struct.it_interval.tv_sec = QUANTUM;
    ...
    while (1)
    {
        ...
        case KVM_EXIT_INTR:
            ...
            // change the VM based on count
            if(vm == vm1)
            {
                vm1_count++;
                current_vm = 0;

                // if VM1 count reached to 7 change it to VM2
                if(vm1_count == 7)
                {
                    current_vm = 1;
                    vm1_count = 0;
            ...
            else
            {
                ...
                // if VM2 count reached to 3 change it to VM1
                if(vm2_count == 3)
                {
                    current_vm = 2;
                    vm2_count = 0;
        ...
        // change VM based on current_vm value
        if(current_vm == 1)
            vm = vm2;
